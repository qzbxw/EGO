...
from core.llm_backend import get_llm_provider, EgoGeminiProvider, LLMProvider
from core.tools import EgoSearch, EgoCalc, EgoWiki, AlterEgo, EgoCodeExec, EgoTube, EgoMemory, ManagePlan
...
class EgoRequest(BaseModel):
    ...
    current_plan: Optional[SessionPlan] = Field(None, description="The active session plan if exists.")
    current_date: Optional[str] = Field(None, description="The current date from the client.")
    user_profile: Optional[str] = Field(None, description="The persistent user profile summary.")
...
async def process_files(request: Request, files: List[UploadFile], cached_files: List[CachedFile], backend: LLMProvider) -> List[Any]:
    """
            ...
            try:
                if is_large_media:
                    ...
                    try:
                        ...
                        if not is_large_media: # Logic inversion check: we set delete=not is_large_media
                            pass
                        # Actually, checking if file exists is safer since delete=not is_large_media means delete=False if large.
                        if os.path.exists(tmp.name) and not tmp.delete:
                             try:
                                 os.unlink(tmp.name)
                ...
                elif mime in ['application/pdf', 'text/plain', 'text/markdown', 'text/csv'] or mime.startswith('text/'):
                ...
                parts.append(f"[File {name} processing failed: {str(e)}]")
                ...
                async with request.app.state.s3_session.client('s3', **request.app.state.s3_params) as s3:
                    ...
                    with NamedTemporaryFile(mode='w+b', delete=True) as tmp:
                        ...
                        if is_large_media:
                            ...
                            content = await _process_pdf(tmp.name, name)
                            parts.append({"type": "file", "name": name, "content": content})
                        elif mime in ['text/plain', 'text/markdown', 'text/csv'] or mime.startswith('text/'):
                            content = await _process_text(tmp, name)
                            parts.append({"type": "file", "name": name, "content": content})
                        else:
...
async def health_check():
    ...
    if vector_memory:
        try:
            ...
            async with vector_memory.pool.acquire() as conn:
    ...
    try:
        ...
        test_response = await default_backend.generate(
            prompt="Health check", 
            max_tokens=5,
            temperature=0.0
        )
...
async def generate_thought(request: Request):
    ...
    async def event_generator():
        try:
            ...
            prompt_parts = await process_files(request, files_data, ego_req.cached_files, backend=backend)
            ...
            thought_json, usage = await ego_instance.generate_thought(
                ...
                current_plan=ego_req.current_plan, current_date=ego_req.current_date,
                user_profile=ego_req.user_profile
            )

            # 1. Yield the thought and header immediately
            if thought_json and 'thoughts_header' in thought_json and thought_json['thoughts_header']:
            ...
            if tool_calls:
                ...
                def get_tool_timeout(tool_name: str) -> float:
                    timeouts = {"EgoTube": 600.0, "EgoSearch": 120.0, "EgoCodeExec": 300.0, "EgoWiki": 60.0, "EgoMemory": 45.0}
                ...
                async def execute_tool_task(tool_call: dict, index: int, total: int):
                    ...
                    # Progress: Starting
                    progress_header = f"Using {tool_name}... ({index}/{total})" if total > 1 else f"Using {tool_name}..."
                    await event_queue.put({
                        "type": "tool_progress",
                        "data": {"tool_name": tool_name, "call_id": call_id, "progress": f"{index}/{total}", "header": progress_header, "status": "running"}
                    })
                    ...
                    try:
                        ...
                        result = await asyncio.wait_for(tool.use(tool_query, user_id=ego_req.user_id), timeout=get_tool_timeout(tool_name))
                        result_str = str(result)
                        
                        # Progress: Completed
                        comp_header = f"{tool_name} completed ({index}/{total})" if total > 1 else f"{tool_name} completed"
                        await event_queue.put({
                            "type": "tool_progress",
                            "data": {"tool_name": tool_name, "call_id": call_id, "progress": f"{index}/{total}", "header": comp_header, "status": "completed"}
                        })
                        
                        # Output - Do not add Markdown prefix if it's a LOCAL_TOOL_SIGNAL (for Go backend interception)
                        output_content = result_str if result_str.startswith("LOCAL_TOOL_SIGNAL:") else f"**Result:**\n{result_str}"
                        ...
                        await event_queue.put({
                            "type": "tool_progress",
                            "data": {"tool_name": tool_name, "call_id": call_id, "progress": f"{index}/{total}", "header": f"{tool_name} failed", "status": "failed"}
                        })
                        await event_queue.put({"type": "tool_error", "data": {"type": "tool_error", "tool_name": tool_name, "call_id": call_id, "error": err_msg}})

                # Start all tools in parallel
                worker_tasks = [asyncio.create_task(execute_tool_task(tc, i, len(tool_calls))) for i, tc in enumerate(tool_calls, 1)]
...
async def clear_memory(req: ClearMemoryRequest):
    ...
    try:
        if not vector_memory:
            return JSONResponse(content={"status": "error", "message": "Memory system not initialized"}, status_code=503)
        
        if not user_id:
            return JSONResponse(content={"status": "error", "message": "user_id is required"}, status_code=400)
        ...
        if vector_memory._pool:
            async with vector_memory._pool.acquire() as conn:
                result = await conn.execute("DELETE FROM ego_memory WHERE user_id = $1", str(user_id))
            ...
            return JSONResponse(content={"status": "error", "message": "Database pool not initialized"}, status_code=503)
    except Exception as e:
...
async def delete_session_vectors(req: DeleteSessionVectorsRequest):
    ...
    try:
        ...
        if not user_id or not session_id:
            return JSONResponse(content={"status": "error", "message": "user_id and session_id are required"}, status_code=400)
        ...
        if vector_memory._pool:
            async with vector_memory._pool.acquire() as conn:
                ...
                rows_deleted = int(result.split()[-1]) if result else 0
                log.info(f"Deleted {rows_deleted} memory entries for user {user_id}, session {session_id}")
                return JSONResponse(content={"status": "success", "rows_deleted": rows_deleted})
        else:
            return JSONResponse(content={"status": "error", "message": "Database pool not initialized"}, status_code=503)
    except Exception as e:
...
async def generate_profile_summary(req: ProfileSummaryRequest):
    try:
        ...
        return JSONResponse(content={"summary": req.current_profile}) # Fallback to existing profile
...
async def embed_text(req: EmbeddingRequest):
    ...
    try:
        if not hasattr(default_backend, 'embed'):
            return JSONResponse(content={"error": "Backend does not support embeddings"}, status_code=501)
