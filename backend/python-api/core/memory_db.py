...
class VectorMemory:
        ...
        async with self._init_lock:
            ...
            try:
                ...
                async with self._pool.acquire() as conn:
                    ...
                    try:
                        await conn.execute(
                            "DROP INDEX IF EXISTS ego_memory_vec_cos_ivfflat"
                        )
                    ...
                    try:
                        ...
                        await conn.execute(
                            f"CREATE INDEX IF NOT EXISTS ego_memory_vec_hnsw_idx ON ego_memory USING hnsw (vector vector_cosine_ops) WITH (m = 16, ef_construction = 64)"
                        )
                        log.info("Created HNSW vector index successfully.")
                    except asyncpg.PostgresError as e:
                        log.warning(f"Could not create HNSW vector index (this is non-critical): {e}")
    ...
    async def _get_cached_embeddings(self, texts: List[str], task_type: str = "RETRIEVAL_DOCUMENT") -> Dict[str, Optional[List[float]]]:
        """
    ...
    async def _cache_embeddings(self, text_embeddings: Dict[str, List[float]], task_type: str = "RETRIEVAL_DOCUMENT") -> None:
        """
        ...
        try:
            ...
            if texts_to_embed:
                if hasattr(self.backend, 'batch_embed'):
                    embeddings = await self.backend.batch_embed(texts_to_embed, task_type, self._dim)
            ...
            for text in valid_texts:
                ...
                if vec is None:
                    log.warning(f"No embedding found for text: '{text[:50]}...'")
                    continue
            ...
            log.info(f"Added {len(rows)} new entries to memory for user '{user_id}' (cache hits: {len([t for t in valid_texts if cached.get(t)])}).")
        except Exception as e:
    ...
    async def search(
        ...
        try:
            ...
            qvec = await self.backend.embed(query, task_type="RETRIEVAL_QUERY", output_dimensionality=self._dim)
            ...
            log.error(f"Hybrid search failed for user '{user_id}': {e}. Falling back to semantic-only.", exc_info=True)
            ...
            try:
                ...
                log.error(f"Fallback semantic search also failed for user '{user_id}': {e2}", exc_info=True)
                return []
