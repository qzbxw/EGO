...
func (db *DB) DeleteSession(sessionUUID string, userID int) (err error) {
	...
	// This is a defensive query that only runs if the table exists. Failure here is non-critical.
	if _, err = tx.Exec(`
        DO $$
        BEGIN
            IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'ego_memory') THEN
                DELETE FROM ego_memory WHERE user_id = $1::text AND session_id = $2::text;
            END IF;
        END$$;
	...
	if err = tx.Get(&remainingSessions, `SELECT COUNT(*) FROM chat_sessions WHERE user_id = $1`, userID); err == nil && remainingSessions == 0 {
		if _, err2 := tx.Exec(`
            ...
            BEGIN
                IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'ego_memory') THEN
                    DELETE FROM ego_memory WHERE user_id = $1::text;
                END IF;
            END$$;
        `, userID); err2 != nil {
