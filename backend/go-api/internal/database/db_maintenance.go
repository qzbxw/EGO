...
func (db *DB) GetMaintenanceMode() (*MaintenanceMode, error) {
	...
	query := `
        SELECT id, is_enabled, message, bypass_token, enabled_at, disabled_at, created_at, updated_at
...
func (db *DB) IsMaintenanceModeEnabled() (bool, error) {
	var isEnabled bool
	query := `SELECT is_enabled FROM maintenance_mode ORDER BY id DESC LIMIT 1`

	err := db.Get(&isEnabled, query)
	if err != nil {
		return false, fmt.Errorf("failed to check if maintenance mode is enabled: %w", err)
	}
...
func (db *DB) EnableMaintenanceMode(message string) (string, error) {
	...
	_, err = db.Exec(query, bypassToken, now, message)
	if err != nil {
