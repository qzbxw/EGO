...
func newFileProcessor(db *database.DB, s3 *storage.S3Service) *fileProcessor {
	return &fileProcessor{db: db, s3Service: s3}
}
	...
	for _, att := range attachments {
		...
		default:
			...
			for idx, chunkText := range chunks {
				...
				vec, err := localEmbedTextFallback(chunkText)
				if err != nil {
					log.Printf("[VECTORS] Failed to embed chunk for %s: %v", att.FileName, err)
					continue
				}
