<script lang="ts">
	...
	async function handleGoogleCallback(response: CredentialResponse) {
		...
		try {
			...
			if (res && res.access_token && res.refresh_token && res.user) {
				...
				await goto('/authenticating?redirect=' + encodeURIComponent('/chat'), { replaceState: true });
			} else {
	...
	$effect(() => {
		try { renderButton(); } catch {}
	});
</script>
<div class="relative w-full flex justify-center items-center min-h-[44px]">
	{#if isLoading}
        <div class="absolute inset-0 flex items-center justify-center animate-pulse text-text-secondary">{$_('login.loading_button')}</div>
	{/if}
