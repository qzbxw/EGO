<script lang="ts">
	...
	import { X, Maximize2, Minimize2, Download, Play, Code as CodeIcon, RefreshCw, ExternalLink } from 'lucide-svelte';
...
>
	<!-- Header -->
	<div class="flex items-center justify-between border-b border-white/5 bg-white/5 px-4 py-3 backdrop-blur-xl transition-colors duration-300 hover:bg-white/10">
		<div class="flex items-center gap-3 overflow-hidden">
			<div class="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-br from-white/10 to-white/5 shadow-inner ring-1 ring-white/10">
				{#if isHtml}
			...
			<div class="flex flex-col">
				<span class="font-bold tracking-tight text-text-primary text-sm truncate max-w-[200px]">{canvasStore.title}</span>
				<span class="text-[10px] font-medium text-text-secondary uppercase tracking-wider">{canvasStore.language}</span>
			</div>
		</div>
