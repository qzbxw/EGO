...
<div
	class="relative flex min-h-screen w-full flex-col overflow-hidden bg-primary selection:bg-accent selection:text-white transition-colors duration-700 ease-in-out"
>
	<!-- Ambient Background -->
	<div class="absolute inset-0 z-0 overflow-hidden pointer-events-none">
		<div
			class="bg-circle absolute -left-[10%] -top-[10%] h-[60vh] w-[60vh] opacity-40 blur-[80px] md:blur-[120px] transition-all duration-1000 animate-pulse-slow"
		></div>
		<div
			class="bg-circle secondary absolute -bottom-[10%] -right-[10%] h-[60vh] w-[60vh] opacity-40 blur-[80px] md:blur-[120px] transition-all duration-1000 animate-pulse-slow"
	...
	>
		...
		>
			<div class="relative z-10 transition-transform duration-500 ease-spring group-hover:rotate-[360deg]">
	...
	<div class="z-10 flex flex-1 items-center justify-center p-4 sm:p-6">
		...
		>
			<!-- Glass Card -->
			<div
				class="relative overflow-hidden rounded-[2rem] border border-white/10 bg-secondary/30 p-8 shadow-2xl backdrop-blur-2xl transition-all duration-500 hover:shadow-[0_0_40px_rgba(var(--color-accent-rgb),0.1)] hover:bg-secondary/40 md:p-10"
			>
				<!-- Header -->
				<div class="mb-10 text-center">
					<div in:fly={{ y: 20, duration: 800, delay: 200, easing: cubicOut }}>
						<button class="mx-auto mb-6 block transition-transform duration-500 hover:scale-110 hover:rotate-6 active:scale-90 active:rotate-12 cursor-default">
							<img
				...
				<form onsubmit={handleLogin} class="space-y-6" novalidate>
					<div in:fly={{ y: 20, duration: 800, delay: 500, easing: cubicOut }} class="group/input">
						...
						<div class="relative transition-all duration-300 ease-out focus-within:-translate-y-1 focus-within:scale-[1.02]">
							<div
								class="absolute inset-y-0 left-0 flex items-center pl-4 text-text-secondary transition-all duration-300 group-focus-within/input:text-accent group-focus-within/input:scale-110 group-focus-within/input:-rotate-12"
							>
								<User class="h-5 w-5" />
							</div>
							<input
						...
						{#if errors.username}
							<p class="mt-2 pl-1 text-xs font-bold text-red-500 animate-pulse" transition:fly={{ y: -5, duration: 200 }}>
								{errors.username}
							</p>
						{/if}
					</div>
					...
					<div in:fly={{ y: 20, duration: 800, delay: 600, easing: cubicOut }} class="group/input">
						...
						<div class="relative transition-all duration-300 ease-out focus-within:-translate-y-1 focus-within:scale-[1.02]">
							...
							<button
								...
								class="absolute inset-y-0 right-0 flex items-center pr-4 text-text-secondary transition-all duration-300 hover:text-text-primary hover:scale-110 active:scale-90"
							>
								{#if showPassword}
						...
						>
							...
							<span class="relative flex items-center justify-center gap-3">
								...
								{:else}
									...
									<ArrowRight
										class="h-5 w-5 transition-transform duration-300 ease-spring group-hover:translate-x-2"
									/>
								{/if}
							</span>
						</button>
				...
				>
					<div class="h-px flex-1 bg-gradient-to-r from-transparent via-tertiary to-transparent opacity-50"></div>
				...
				<!-- Social Login -->
				<div in:fly={{ y: 20, duration: 800, delay: 900, easing: cubicOut }} class="transition-transform duration-300 hover:scale-[1.02] active:scale-[0.98]">
					<GoogleLogin />
				</div>
				...
				>
					<p class="text-sm font-medium text-text-secondary">
						...
						<a
							href="/register"
							class="group ml-1 relative inline-block font-bold text-accent transition-colors hover:text-accent-hover"
						>
							<span class="relative z-10">{$_('login.register')}</span>
							<span class="absolute bottom-0 left-0 h-[2px] w-0 bg-accent transition-all duration-300 ease-out group-hover:w-full"></span>
						</a>
					</p>

					<div class="mt-6 flex justify-center gap-4 text-[11px] font-medium text-text-secondary/50">
						<div class="flex items-center gap-1">
							<span>{$_('auth.agree_prefix')}</span>
							<a href="/terms" target="_blank" class="hover:text-text-primary transition-colors hover:underline decoration-text-primary/30 underline-offset-2"
								>{$_('legal.terms_title')}</a
							>
						</div>
						<div class="flex items-center gap-1">
							<span>{$_('auth.and')}</span>
							<a href="/privacy" target="_blank" class="hover:text-text-primary transition-colors hover:underline decoration-text-primary/30 underline-offset-2"
