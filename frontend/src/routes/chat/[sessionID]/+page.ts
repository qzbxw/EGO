  ...
  try {
    ...
    for (const log of historyLogs) {
      ...
      if (log.final_response) {
        ...
        const normalizedThoughts = log.thoughts?.map((t: any) => ({
          ...t,
          header: t.header || t.thoughts_header || (t.type === 'thought' ? 'Thinking' : 'Using tool'),
        ...
        for (const m of existing.messages) {
          ...
          if (!m.logId || !serverLogIds.has(m.logId)) {
            ...
            const isDuplicate = merged.some((x) =>
                x.id === m.id ||
                (x.author === m.author && x.text === m.text && x.author === 'user')
            );
            if (!isDuplicate) {
              merged.push(m);
            }
